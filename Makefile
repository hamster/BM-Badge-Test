PROJECT_NAME     := bm-badge-test
TARGETS          := bm-badge-test
...
$(OUTPUT_DIRECTORY)/bm-badge-test.out:
  LINKER_SCRIPT  := $(PROJ_DIR)/config/bm_badge_gcc_nrf52.ld
...
SRC_FILES += \
  ...
  \
  $(PROJ_DIR)/utility.c \
  $(PROJ_DIR)/modules/drv_ili9341.c \
...
merge: $(PROJECT_NAME)
	@echo Merging softdevice and user hex
	@$(MERGEHEX) -m $(SOFTDEVICE) $(OUTPUT_DIRECTORY)/$(PROJECT_NAME).hex -o $(PROJECT_NAME)-merged.hex
	@$(SIZE) $(PROJECT_NAME)-merged.hex

flash_merge: merge
	@echo Flashing: $<
	$(NRFJPROG) --program $(PROJECT_NAME)-merged.hex -f nrf52 --sectorerase
	$(NRFJPROG) --reset -f nrf52

uf2conv_debug: $(OUTPUT_DIRECTORY)/$(PROJECT_NAME).hex
	@echo Building UF2 file: $<
	$(PYTHON3) $(UF2CONV) $< -c -f 0xADA52840 -o $(OUTPUT_DIRECTORY)/$(PROJECT_NAME).uf2
